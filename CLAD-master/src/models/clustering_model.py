...
def set_text_data(train_x, train_y, batch_size=32):

    num_data = train_x.shape[0]
    embed_length = train_x.shape[1]
    data = TensorDataset(train_x, train_y)
    dataloader = DataLoader(data, batch_size=batch_size)
    return dataloader, num_data, embed_length
...
class Clustering_Module():
    def __init__(self,
        ...
        if (dataset_name in config.cps_datasets):
            ...
            self.dataloader, self.num_data, self.embed_length = set_text_data(
                train_x, train_y, batch_size=batch_size)
            self.height = 0
            self.width = 0
        ...
        self.encoder = Encoder(dataset_name=dataset_name,
                               ...
                               height=self.height,
                               width=self.width,
                               embed_length=self.embed_length,
    ...
    def train(self, epochs):
        ...
        for epoch in range(1):
            ...
            predicted, actual, _ = self.predict()
            delta_label = float(
    ...
    def predict(self):
        ...
        return torch.cat(features).max(1)[1], torch.cat(
            actual).long(), torch.cat(features)
...
class Encoder(nn.Module):
    def __init__(self, dataset_name, cluster_type, height, width, embed_length,
                 n_components, n_hidden_features):
        super(Encoder, self).__init__()
        self.dataset_name = dataset_name
        if (dataset_name in config.text_datasets):
        ...
        elif (cluster_type == 'linear_word'):
            self.dropout = nn.Dropout(p=0.1)
            self.encoder_net = nn.Sequential(
                nn.Linear(self.embed_length, 500), nn.ReLU(),
                nn.Linear(500, 500), nn.ReLU(), nn.Linear(500, 2000),
                nn.ReLU(), nn.Linear(2000, n_hidden_features))
    ...
    def forward(self, x):
        if (self.cluster_type == 'linear_word'):
            x = x.reshape(-1, self.embed_length)
        out = self.dropout(x)
...
class Decoder(nn.Module):
    def __init__(self, dataset_name, cluster_type, height, width, embed_length,
        ...
        if (cluster_type == 'linear'):
            ...
            self.decoder_net = nn.Sequential(
                nn.Linear(n_hidden_features, 2000), nn.ReLU(),
                nn.Linear(2000, 500), nn.ReLU(), nn.Linear(500, 500),
                nn.ReLU(), nn.Linear(500, self.embed_length))
        elif (cluster_type == 'cvae'):
    ...
    def forward(self, x):
        ...
        elif (self.cluster_type == 'linear_word'):
            out = self.decoder_net(x)
        ...
        if (self.dataset_name in config.cps_datasets):
            ...
            out = out.reshape(-1, self.embed_length)
